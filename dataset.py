...
class Dataset(object):
	...
	def __init__(self,tr_filename,va_filename=None,batch_size=None,shuffle=True):
		...
		self.value_plhd = tf.placeholder(dtype = tf.float32,
		...
		self.train_dataset = tf.data.Dataset.from_tensor_slices((self.feat_plhd,
			self.value_plhd, 
			self.label_plhd))

		# create validating dataset
		if va_filename is not None:
			self.val_dataset = tf.data.Dataset.from_tensor_slices((self.feat_plhd,
				self.value_plhd,
				self.label_plhd))
	...
	def _load_and_parse_raw_data(self,filename):
		...
		for line in raw_data:
			_,feat,val,label = self._parse_line(line)
			feature_data.append(feat)
			label_data.append(label)
			value_data.append(val)
		self.all_data.append({ "feature" : np.array(feature_data), 
